...
import { formatPrice, productImage } from "@/lib/shopify";

export default function Index() {
  const { data: products, isLoading } = useShopifyProducts(undefined, 3);
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12 items-start">
          <h2 className="text-3xl md:text-4xl font-light text-foreground leading-snug">
            Everyday essentials, made to be worn every day.
          </h2>
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {featured.map((product) => (
                ...
                >
                  ...
                  <h3 className="text-base font-light text-foreground mb-1">{product.node.title}</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <Link to="/collections/core" className="relative overflow-hidden group block">
            ...
            <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-colors duration-500 flex flex-col justify-between p-8">
              ...
              <h3 className="text-2xl md:text-3xl font-light text-white">The Sock Collection</h3>
            </div>
          </Link>
          <Link to="/collections/sets-and-pairs" className="relative overflow-hidden group block">
            ...
            <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-colors duration-500 flex flex-col justify-between p-8">
              ...
              <h3 className="text-2xl md:text-3xl font-light text-white">Everyday Essentials</h3>
            </div>
          </Link>
        </div>
      </section>
