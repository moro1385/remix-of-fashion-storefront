...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Create Supabase service role client to fetch order and update it
    ...
    if (order.user_id !== user.id) {
      return new Response(JSON.stringify({ error: 'Forbidden: This is not your order' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    if (updateError) {
       return new Response(JSON.stringify({ error: 'Failed to update order payment reference' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
